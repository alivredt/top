...
@elhzeyba.on(events.NewMessage(outgoing=True, pattern=r"^\.غرد (\d+) (@?\S+)$"))
...
async def ze_allnshr(elhzeyba, sleeptimet, message):
    ...
    while yaAhmed:
        ...
        await asyncio.sleep(sleeptimet)
@elhzeyba.on(events.NewMessage(outgoing=True, pattern=r"^\.غرد_كروبات (\d+)$"))
async def Ahmed(event):
    await event.delete()
...
async def Ahmed(event):
    ...
    try:
        ...
        pass
@elhzeyba.on(events.NewMessage(outgoing=True, pattern='.ايقاف التغريد'))
async def stop_ze(event):
    global yaAhmed
    yaAhmed = False
    await event.edit("**۞︙ تم ايقاف التغريد التلقائي بنجاح ✓** ")
...
`.غرد` عدد الثواني معرف الكروب :
~ للنشر في المجموعة التي وضعت معرفها مع عدد الثواني

`.غرد_كروبات` عدد الثواني : 
...
===== 🔱Ali Ahmed 🔱 =====
    **"""
        ...
        try:
            ...
            pass
print('تم تشغيل نشر التلقائي لسورس علي احمد ')
